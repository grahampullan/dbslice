var dbslice = (function (exports) {
	...
	var threeSurf3d = {
	  ...
	  update: function update(element, geometry, layout) {
	    ...
	    var width = container.node().offsetWidth,
	        height = layout.height; // Compute normals for shading

	    geometry.computeFaceNormals();
	    geometry.computeVertexNormals(); // Use MeshPhongMaterial for a reflective surface
	    ...
	    var scene = new THREE.Scene(); // Add background colour

	    scene.background = new THREE.Color(0xefefef); // Add Mesh to scene

	    scene.add(new THREE.Mesh(geometry, material)); // Create renderer
	    ...
	    renderer.setSize(width, height); // Set target DIV for rendering
	    //var container = document.getElementById( elementId );

	    div.node().appendChild(renderer.domElement); // Define the camera

	    var camera = new THREE.PerspectiveCamera(60, 1, 0.1, 10);
	    camera.position.z = 2; // Add controls 
	...
	function threeMeshFromStruct(data) {
	  ...
	  var scalefac = 1. / d3.max([xrange, yrange, zrange]); // Use d3 for color scale 
	  // vMinAll=0.4;
	  // vMaxAll=1.1;
	  ...
	  for (nds = 0; nds < nDataSets; ++nds) {
	    ...
	    var nverts = n * m; // Add grid vertices to geometry
	    ...
	    for (var j = 0; j < m - 1; j++) {
	      for (var i = 0; i < n - 1; i++) {
	        ...
	        var face2 = new THREE.Face3(n2 + noffset, n3 + noffset, n0 + noffset); // face1.vertexColors[0] = new THREE.Color( color( v[n0] ) );
	...
	var d3ContourStruct2d = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    var thresholds = d3.range(vMinAll, vMaxAll, (vMaxAll - vMinAll) / 21); // colour scale 
	    ...
	    for (var nds = 0; nds < nDataSets; ++nds) {
	      ...
	      v = data.surfaces[nds].v;
	      m = data.surfaces[nds].size[0];
	      n = data.surfaces[nds].size[1]; // configure a projection to map the contour coordinates returned by
	      ...
	      var contours = d3.contours().size([n, m]).smooth(true).thresholds(thresholds); // make and project the contours
	...
	var d3LineSeries = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    var yscale0 = d3.scaleLinear().range([height, 0]).domain(yRange); //var colour = ( layout.colourMap === undefined ) ? d3.scaleOrdinal( d3.schemeCategory10 ) : d3.scaleOrdinal( layout.colourMap );
	    //if ( layout.cSet !== undefined) colour.domain( layout.cSet );
	    ...
	    allSeries.enter().each(function () {
	      ...
	      var seriesLine = series.append("g").attr("class", "plotSeries").attr("series-name", function (d) {
	        ...
	        return colour(d.cKey);
	      }).style("fill", "none").style("stroke-width", "2.5px") //.attr( "clip-path", "url(#clip)")
	...
	var d3Scatter = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    points.transition() //.duration(5000)
	...
	var cfD3BarChart = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    var group = dim.group(); //var items = group.top( Infinity );
	    ...
	    bars.transition().attr("width", function (v) {
	      ...
	      return y(v.key);
	    }).attr("height", y.bandwidth()) // change colour depending on whether the bar has been selected
	...
	var cfD3Histogram = {
	  make: function make(element, data, layout) {
	    ...
	    var gBrush = svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").attr("class", "brush").call(brush); // style brush resize handle
	...
	var cfLeafletMapWithMarkers = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    container.select(".plotArea").remove(); // always make a new map
