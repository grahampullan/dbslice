var dbslice = (function (exports) {
	    ...
	    highlightDataPoint: function highlightDataPoint(element, plot, d) {
	      ...
	      switch (plot.plotFunc.name) {
	        case "cfD3Scatter":
	          ...
	          d3.select(element).selectAll("circle[task-id='" + d.taskId + "']").style("opacity", 1.0).attr("r", 7);
	          break;
	...
	var cfD3Histogram = {
	  ...
	  update: function update(element, data, layout) {
	    ...
	    var svg = container.select("svg"); // Get the required data.
	  ...
	  setupInteractivity: {
	    addBrush: function addBrush(svg) {
	      ...
	      function dragmove() {
	        ...
	        cfD3Histogram.setupInteractivity.updateBrush(svg); // Update the data selection
