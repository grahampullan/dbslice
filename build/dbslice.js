var dbslice = (function (exports) {
  ...
  var cfD3Histogram = {
    ...
    make: function make(ctrl) {
      ...
      ctrl.figure.select("svg.plotArea").select("g.markup").append("g").attr("class", "extent"); // cfD3Histogram.setupPlot.appendHorizonalSelection(ctrl.figure.select(".bottomAxisControlGroup"), ctrl)
    ...
    draw: {
      plotDataExtent: function plotDataExtent(ctrl, items) {
        ...
        var bars = ctrl.figure.select("svg.plotArea").select("g.markup").select("g.extent").selectAll("rect").data(items); // Finally append any new bars with 0 height, and then transition them to the appropriate height

        var newBars = bars.enter();
        newBars.append("rect").attr("transform", a.startState).attr("x", 1).attr("width", a.width).attr("height", 0).style("fill", "black").attr("opacity", 0.1).transition().delay(t.enterDelay).duration(t.duration).attr("height", a.height).attr("transform", a.finishState); // Now move the existing bars.

        bars.transition().delay(t.updateDelay).duration(t.duration).attr("transform", a.finishState).attr("x", 1).attr("width", a.width).attr("height", a.height); // Remove any unnecessary bars by reducing their height to 0 and then removing them.

        bars.exit().transition().duration(t.duration).attr("transform", a.startState).attr("height", 0).remove();
      },
      // plotDataExtent
      plotCurrentSelection: function plotCurrentSelection(ctrl, items) {
        ...
        var bars = ctrl.figure.select("svg.plotArea").select("g.data").selectAll("rect").data(items); // Finally append any new bars with 0 height, and then transition them to the appropriate height

        var newBars = bars.enter();
        newBars.append("rect").attr("transform", a.startState).attr("x", 1).attr("width", a.width).attr("height", 0).style("fill", a.fill).attr("opacity", 1).transition().delay(t.enterDelay).duration(t.duration).attr("height", a.height).attr("transform", a.finishState); // Now move the existing bars.
      ...
      isRegroupNeeded: function isRegroupNeeded(ctrl) {
        var flag = ctrl.view.gVar != ctrl.view.xVarOption.val || ctrl.view.gClr != color.settings.variable; // Update the 'gVar' and 'gClr' flags for next draw.				
      ...
      regroup: function regroup(ctrl) {
        ...
        g.selectAll("rect").transition().duration(500).attr("transform", a.startState).attr("height", 0).remove().end().then(function () {
          // Redo the plot tools
          ...
          cfD3Histogram.addInteractivity.addBrush.updateBrush(ctrl); // Update any bin controls.

          cfD3Histogram.addInteractivity.addBinNumberControls.updateMarkers(ctrl); // All elements were removed. Update teh chart.
      ...
      update: function update(ctrl) {
        ...
        var filterItems = h.getFilteredItems(ctrl); // Background data extent

        cfD3Histogram.draw.plotDataExtent(ctrl, backgroundItems); // Handle the entering/updating/exiting of bars.

        cfD3Histogram.draw.plotCurrentSelection(ctrl, filterItems); // Handle the axes.
    ...
    helpers: {
      ...
      highlight: function highlight(ctrl, allDataPoints) {
        ...
        var highlightedData = cfD3Histogram.helpers.getItems(highlightedBins, color.settings.variable); // Draw the highlighted data.
  ...
  var cfD3Line = {
    ...
    update: function update(ctrl) {
      ...
      plotHelpers.setupPlot.general.rescaleSvg(ctrl); // Setup the plot tools

      cfD3Line.setupPlot.setupPlotTools(ctrl); // Call the refresh function which readjust the plot elements.
    ...
    refresh: function refresh(ctrl) {
      if (ctrl.data.compatible.length > 0) {
        ...
        var getTaskId = function getTaskId(d) {
        ...
        var clipPath = "url(#" + ctrl.figure.select("svg.plotArea").select("clipPath").attr("id") + ")"; // Assign the data
        ...
        allSeries.enter().each(function () {
          ...
          var seriesLine = series.append("g").attr("class", "plotSeries").attr("task-id", getTaskId).attr("clip-path", clipPath).append("path").attr("class", "line").attr("d", draw).style("stroke", getColor).style("fill", "none").style("stroke-width", "2.5px"); // Add a tooltip to this line
        ...
        allSeries.selectAll("path.line").transition().duration(ctrl.view.transitions.duration).attr("d", draw).style("stroke", getColor);
    ...
    setupPlot: {
      ...
      setupPlotTools: function setupPlotTools(ctrl) {
        ...
        plotHelpers.setupTools.go(ctrl); // Make the required line tool too!
    ...
    helpers: {
      ...
      updateManualSelections: function updateManualSelections(ctrl) {
        var g = ctrl.figure.select("svg.plotArea").select("g.data"); // Color in selected circles.
  ...
  var addMenu = {
    addPlotControls: {
      ...
      make: function make(containerDOM, containerCtrl) {
        ...
        var plotRowTitle = d3.select(containerDOM); // Create the config element with all required data.

        var config = addMenu.addPlotControls.createConfig(containerDOM, containerCtrl); // Make the button.

        addMenu.addPlotControls.makeButton(config); // First create the ids of the required inputs
      ...
      enableDisableSubmitButton: function enableDisableSubmitButton(config) {
        ...
        d3.select(config.menuContainer.node().parentElement).selectAll("button[type='submit']").each(function () {
      ...
      onPlotTypeChangeEvent: function onPlotTypeChangeEvent(config, selectDOM, variable) {
        ...
        switch (config.newCtrl.plottype) {
          ...
          case "cfD3BarChart":
            // yProperty required.
            h.addUpdateMenuItemObject(config, 'yProperty', dbsliceData.data.metaDataProperties, "Select variable"); // xProperty must not be present.
          ...
          case "cfD3Histogram":
            // xProperty required.
            h.addUpdateMenuItemObject(config, "xProperty", dbsliceData.data.dataProperties, "Select variable"); // yProperty must not be present.
      ...
      submitNewPlot: function submitNewPlot(config) {
        ...
        plotRow.plots.push(plotToPush); // Add the new plot to the session object. How does this know which section to add to? Get it from the parent of the button!! Button is not this!
      ...
      clearMenu: function clearMenu(config) {
        ...
        addMenu.helpers.resetVariableMenuSelections(config, "slice"); // Remove the select menus from the view.

        addMenu.helpers.removeMenuItemObject(config, "xProperty");
        addMenu.helpers.removeMenuItemObject(config, "yProperty");
        addMenu.helpers.removeMenuItemObject(config, "slice"); // Update the menus so that the view reflects the state of the config.
      ...
      makeButton: function makeButton(config) {
        ...
        config.ownerButton = config.ownerContainer.append("button").attr("style", "display:inline").attr("class", "btn btn-success float-right").html(buttonLabel);
      },
      // makeButton
      ...
      allPlotRows.each(function () {
        ...
        allPlotTitles.each(function () {
          ...
          if (removePlotButton.empty()) {
            ...
            var btn = ctrlGroup.append("button").attr("class", "btn btn-danger float-right").html("x"); // Move the button in front of other elements.
          ...
          d3.select(this).select(".btn-danger").on("click", function () {
            ...
            var thisPlot = d3.select(plotCardDOM).select(".plot"); // plotCardDOM -> plotWrapper -> plotRowBody -> plotRow
    ...
    addPlotRowControls: {
      ...
      make: function make(containerId, buttonId) {
        ...
        var config = addMenu.addPlotRowControls.createConfig(containerId, buttonId); // Add or move the actual button.

        addMenu.addPlotRowControls.makeButton(config); // First create the ids of the required inputs
      ...
      submitNewPlotRow: function submitNewPlotRow(config) {
        ...
        addMenu.helpers.resetVariableMenuSelections(config, "type"); // Clear the config
      ...
      enableDisableSubmitButton: function enableDisableSubmitButton(config) {
        ...
        var submitButtonDOM = d3.select(config.menuContainer.node().parentElement).select("button[type='submit']").each(function () {
          this.disabled = disabledFlag;
        });
      },
      // enableDisableSubmitButton
      ...
      makeButton: function makeButton(config) {
        ...
        if (addPlotRowButton.empty()) {
          // Add the button.
          config.ownerButton = config.ownerContainer.append("button").attr("id", config.ownerButtonId).attr("class", "btn btn-info btn-block").html("+"); // Add the interactivity
      ...
      // makeButton
      makeMenuItem: function makeMenuItem(config, variable, options, label) {// addPlotRowControls does not expect any items to be created.
      } // makeMenuItem
    ...
    helpers: {
      makeMenuContainer: function makeMenuContainer(config) {
        ...
        if (config.menuContainer == undefined) {
          config.menuContainer = config.ownerContainer.append("div").attr("class", "card ui-draggable-handle");
      ...
      updateMenus: function updateMenus(config) {
        ...
        var menus = config.menuContainer.selectAll("label").data(config.menuItems); // Handle the entering menus. These require a new 'select' element and its 'option' to be appended/updated/removed.

        menus.enter().append("label").text(function (d) {
          return d.label;
        }).append("select").attr("type", function (d) {
          return d.variable;
        }); // Remove exiting menus.

        menus.exit().remove(); // Update all the menu elements.

        config.menuContainer.selectAll("label").each(function (menuItem) {
      ...
      addUpdateMenuItemObject: function addUpdateMenuItemObject(config, variable, options, label) {
        ...
        config.ownerContainer.selectAll(".warning").remove(); // Only add or update the menu item if some selection variables exist.
        ...
        if (options.length > 1) {
          ...
          addMenu.helpers.removeMenuItemObject(config, variable); // Tell the user that the data is empty.

          var warning = config.ownerContainer.selectAll(".warning");

          if (warning.empty()) {
            config.ownerContainer.append("div").attr("class", "warning").html("No data has been loaded!").attr("style", "background-color:pink;font-size:25px;color:white");
          }
        }
      },
      ...
      resetVariableMenuSelections: function resetVariableMenuSelections(config, variable) {
        config.menuContainer.selectAll("select[type='" + variable + "']").each(function () {
      ...
      enableDisableAllButtons: function enableDisableAllButtons() {
        ...
        document.getElementById("sessionOptions").disabled = false; // "Load session" only available after some data has been loaded.
  ...
  var cfD3BarChart = {
    ...
    draw: {
      plotDataExtent: function plotDataExtent(ctrl, items) {
        ...
        var bars = ctrl.figure.select("svg.plotArea").select("g.markup").selectAll("rect").data(items); // New bars
      ...
      regroup: function regroup(ctrl) {
        ...
        svg.select("g.data").selectAll("rect").transition().duration(500).attr("x", ctrl.tools.xscale(0)).attr("width", 0).remove().end().then(function () {
    ...
    addInteractivity: {
      ...
      addOnMouseClick: function addOnMouseClick(ctrl) {
        ...
        function onClick(d) {
          console.log("on bar click"); // Update the filter selection.
