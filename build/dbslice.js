var dbslice = (function (exports) {
    ...
    helpers: {
        ...
        switch (plot.plotFunc.name) {
          ...
          case "d3Contour2d":
            ...
            if (d3.select(element).attr("task-id") == d.taskId) {
              d3.select(element.parentElement.parentElement.parentElement).attr("class", "card border border-dark");
              d3.select(element.parentElement.parentElement.parentElement).select(".plotTitle").style("font-weight", "bold");
            } // if
      ...
      setDefaultStyle: function setDefaultStyle(element, plot) {
        ...
        switch (plot.plotFunc.name) {
          ...
          case "d3Contour2d":
            // Remove any border
            d3.select(element.parentElement.parentElement.parentElement).attr("class", "card");
            d3.select(element.parentElement.parentElement.parentElement).select(".plotTitle").style("font-weight", "");
            break;
        } // switch
  ...
  var cfD3Scatter = {
    ...
    make: function make(element, data, layout) {
      ...
      cfD3Scatter.addInteractivity.updatePlotTitleControls(element); // Update the plot.

      cfD3Scatter.update(element, data, layout);
    },
    update: function update(element, data, layout) {
      cfD3Scatter.setupSvg(element, data, layout); // Selections for plotting.
      ...
      cfD3Scatter.addInteractivity.addZooming(svg, data); // Add zooming.

      cfD3Scatter.addInteractivity.addSelection(svg); // Highlight any manually selected tasks.

      cfD3Scatter.helpers.updateManualSelections(); // HELPER FUNCTIONS
    ...
    setupSvg: function setupSvg(element, data, layout) {
      ...
      if (svg.empty()) {
        ...
        if (expectedPlotType !== "cfD3Scatter") {
          ...
          curateSvg(); // Add the manual selection toggle to its title.
    ...
    addInteractivity: {
      addTooltip: function addTooltip(svg) {
        ...
        function tipOn(d) {
          ...
          tip.show(d); // Here I want to add cross highlighting.
        ...
        function tipOff(d) {
          ...
          tip.hide(); // Add cross highlighting effect.
    ...
    helpers: {
      ...
      updateManualSelections: function updateManualSelections() {
        ...
        allScatterPlots.each(function () {
          var svg = d3.select(this).select("svg"); // Default color

          svg.selectAll("circle").style("fill", "rgb(31, 119, 180)"); // Color in selected circles.
  ...
  var d3LineSeries = {
    ...
    update: function update(element, data, layout) {
      ...
      allSeries.each(function () {
        var series = d3.select(this).attr("series-name", function (d) {
          return d.label;
        }).attr("task-id", function (d) {
          return d.taskId;
        });
  ...
  var cfDataManagement = {
    cfInit: function cfInit(metadata) {
      ...
      dbsliceData.filteredTaskIds = cfDataManagement.helpers.getTaskIds(metadata); // Store data internally
      ...
      allPlotRows.each(function (d, i) {
        ...
        allPlotTitles.each(function (d, i) {
          ...
          if (addPlotButton.empty()) {
            ...
            ctrlGroup.append("button").attr("class", "btn btn-danger float-right").html("x").on("click", function () {
              ...
              render(dbsliceData.elementId, dbsliceData.session); // Manually realign elements that are moveable - histogram brush.
  ...
  var importExportFunctionality = {
    ...
    importData: {
      ...
      load: function load(file, dataAction) {
        ...
        var extension = file.name.split(".").pop(); // Determine if the input adds new data, or if it replaces the data.
  ...
  function render(elementId, session) {
    ...
    d3.selectAll(".d3-tip").remove(); // THIS CAN CURRENTLY RESOLVE PROBLEMS F THE DATA IS INCOMPATIBLE.
    ...
    addMenu.removePlotControls(); // REPLACE CURRENT DATA OPTION:
    ...
    d3.select("#getSession").on("click", function () {
      sessionInput.click();
  ...
  function makeSessionHeader(element, title, subtitle, config) {
    ...
    sessionMenu.append("a").attr("class", "dropdown-item").attr("href", "#").attr("id", "getSession").html("Load session");
    sessionTitle.append("br");
    sessionTitle.append("br");
